    ...
    try {
      ...
      if (missingFields.length > 0) {
        ...
        if (DEBUG) {
          res.status(400).json({
            ...
            receivedUserInfo: parsedUserInfo,
            missingFields,
            requiredFields
          });
        } else {
      ...
      if (parsedUserInfo.status !== 'ACTIVE') {
        ...
        if (DEBUG) {
          res.status(400).json({
            ...
            receivedStatus: parsedUserInfo.status,
      ...
      if (DEBUG) {
        res.json({
          ...
          ssoData: {
            idp,
            access_token: access_token.substring(0, 10) + '...', // 部分隐藏
            refresh_token: refresh_token.substring(0, 10) + '...', // 部分隐藏
            token_type,
            expires_in,
            state
          }
